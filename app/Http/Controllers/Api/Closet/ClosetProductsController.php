...
//                'shipment_country' => $freeShipment ? 0 : (array_key_exists('country', $requestData['shipment']) && !empty($requestData['shipment']['country']) ? $requestData['shipment']['country'] : 0),
     ...
     * @OA\Post(
     *
     *     path="/v1/filter/closet/{slug}/product",
     *     tags={"Closet"},
    ...
    public function getClosetProducts(Request $request, $closetRef)
    {
        try {
            ...
            $store = self::getCachedStore($closetRef);
            if ($store) {
                $response = self::getCachedClosetProducts($request, $store, '');
                return ApiResponseHandler::success($response, __('messages.general.success'));
            }
            return ApiResponseHandler::failure("Store not found", '', ['not_found' => Constant::Yes]);
        } catch (\Exception $e) {
    ...
    public function getCachedStore($closetRef)
    {
        ...
        return Cache::remember($cacheKey, env('CACHE_REMEMBER_SECONDS'), function () use ($closetRef) {
            return Closet::findByReference($closetRef);
        });
    }
     ...
     * @OA\Get(
     *
     *     path="/v1/closet/{slug}/category/{catSlug}/product",
     *     tags={"Closet"},
